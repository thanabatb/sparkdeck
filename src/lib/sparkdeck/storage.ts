...
function normalizeCounters(rows: CounterRow[] | null): SparkDeckCounters {
  ...
  for (const row of rows ?? []) {
    if (row.name === "spark") {
      base.spark = ensurePositiveInteger(row.next_value, base.spark);
    }

    if (row.name === "task") {
      base.task = ensurePositiveInteger(row.next_value, base.task);
    }

    if (row.name === "build") {
      base.build = ensurePositiveInteger(row.next_value, base.build);
    }
  }
...
export async function writeStorage(data: SparkDeckData): Promise<void> {
  ...
  if (counterError) {
    throwStorageError("Failed to write counters", counterError.message);
  }
}
